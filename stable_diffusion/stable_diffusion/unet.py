...
class Conv2d(nn.Conv2d):
    ...
    def forward(self, *args, **kwargs):
        # rewrite conv2d in order to assign new forward method
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, memory, attn_mask, memory_mask):
        # Self attention
        y = self.norm1(x)
...
class Transformer2D(nn.Module):
    ...
    def forward(self, x, encoder_x, attn_mask, encoder_attn_mask):
...
class UNetModel(nn.Module):
    ...
    def forward(
        ...
        for block in self.up_blocks:
            res_samples = residuals[-len(block.resnets):]
            ...
            x, _ = block(
                ...
                attn_mask=attn_mask,
                encoder_attn_mask=encoder_attn_mask,
                residual_hidden_states=res_samples,
            )
